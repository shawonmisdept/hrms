...
<div class="max-w-3xl mx-auto bg-white rounded border border-gray-300 shadow-sm p-5">
    <h2 class="text-2xl font-semibold text-gray-700 mb-6">Add New Division</h2>
    ...
    <form action="{{ route('divisions.store') }}" method="POST">
        ...
        <div class="mb-4">
            ...
            <textarea name="description" rows="3"
                      class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm px-2 py-1 text-sm">{{ old('description') }}</textarea>
            @error('description')
                <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
            @enderror
        </div>
        ...
        <div class="flex justify-end space-x-2">
            ...
            <button type="submit" class="px-3 py-1 bg-green-600 text-white rounded hover:bg-green-700 text-sm">Submit</button>
        </div>
    </form>
</div>
@endsection
