...
<div class="p-6 w-[80%] mx-auto">
    <div class="bg-white rounded-xl shadow-lg p-8 border border-gray-100">
        ...
        {{-- Title --}}
        <h1 class="text-xl font-semibold mb-6 text-gray-700 border-b pb-2">Edit Salary Grade</h1>
        ...
        {{-- Form --}}
        <form method="POST" action="{{ route('salary-grades.update', $salaryGrade->id) }}"
            ...
            <div class="col-span-1">
                ...
                <select name="status" id="status"
                        class="w-full border border-gray-300 rounded px-4 py-2 text-sm
                               focus:ring-2 focus:ring-green-200 focus:border-green-500 hover:border-green-400"
                        required>
                    <option value="1" {{ old('status', $salaryGrade->status) == 1 ? 'selected' : '' }}>Active</option>
                    <option value="0" {{ old('status', $salaryGrade->status) == 0 ? 'selected' : '' }}>Inactive</option>
                </select>
                @error('status')
                    <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                @enderror
