...
<div class="max-w-3xl mx-auto bg-white p-6 rounded shadow">
    <h2 class="text-lg font-semibold mb-4">Edit Salary Head</h2>

    <form method="POST" action="{{ route('salary_heads.update', $salaryHead->id) }}">
        @csrf
        @method('PUT')
        ...
        <div class="mb-4">
            ...
            <textarea name="description" class="w-full border px-3 py-2 rounded text-sm" rows="2">{{ old('description', $salaryHead->description) }}</textarea>
        </div>

        {{-- Head Type --}}
        <div class="mb-4">
            <label class="block mb-1 font-medium text-sm">Head Type</label>
            <select name="head_type" class="w-full border px-3 py-2 rounded text-sm">
                @php
        ...
        <div class="mb-4">
            ...
            <select name="perquisite" class="w-full border px-3 py-2 rounded text-sm">
        ...
        <div class="mb-4">
            ...
            <select name="disburse" class="w-full border px-3 py-2 rounded text-sm">
        ...
        <div class="mb-6">
            ...
            <select name="status" class="w-full border px-3 py-2 rounded text-sm">
                <option value="1" {{ old('status', $salaryHead->status) == 1 ? 'selected' : '' }}>Active</option>
                <option value="0" {{ old('status', $salaryHead->status) == 0 ? 'selected' : '' }}>Inactive</option>
            </select>
        </div>

        {{-- Buttons --}}
        <div class="flex justify-between">
            <a href="{{ route('salary_heads.index') }}" class="px-4 py-2 bg-gray-500 hover:bg-gray-600 text-white rounded text-sm">Back</a>
            <button type="submit" class="px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white rounded text-sm">Update</button>
        </div>
    </form>
</div>
@endsection
